 ...
 *
 * 3/4/6/8-bit quantization with Walsh-Hadamard rotation that gaussianizes
...
/* Self test: encodes random Gaussian rows, checks cosine similarity above
 * a per-bits floor for 3/4/6/8-bit. Returns 0 on success, non-zero on
 * failure. */
int    ds4_turbo_self_test(void);

/* Accessor for the runtime turbo KV bits configuration (0 = disabled, else
 * 3/4/6/8). Set by ds4.c at engine startup from $DS4_TURBO_KV_BITS so the
 ...
 *   6-bit: half norm + 24B qs                          = 26 B
 *   8-bit: half norm + 32B qs                          = 34 B
 */
