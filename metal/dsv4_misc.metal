...
// Extended args for kernel_dsv4_indexed_mixed_attention_heads8_turbo.
// Layout mirrors ds4_metal_args_dsv4_indexed_attention then appends the
// turbo bits and RoPE tail dim count; the host-side struct
// ds4_gpu_dsv4_indexed_attention_turbo_args matches this exactly.
struct ds4_metal_args_dsv4_indexed_attention_turbo {
    uint32_t n_tokens;
    uint32_t n_head;
    uint32_t n_raw;
    uint32_t raw_cap;
...
// Turbo-aware variant of kernel_dsv4_indexed_mixed_attention_heads8.
//
// Reads `raw_kv` directly as packed TurboQuant bytes (~352 B for 3-bit,
// ~416 B for 4-bit) and inline-decodes each row into the shared
// threadgroup buffer just before consumption. This avoids the
// pre-dequant pass that materializes all n_raw rows into a fp16 scratch
// buffer ahead of time -- under decode (n_tokens == 1) and short
// prefill batches the scratch round-trip dominates the actual attention
// math, so we trade a little extra ALU per row for ~3x less raw KV
// bandwidth and one fewer Metal dispatch per attention call.
//
// `comp_kv` is unchanged: the compressor cache stays fp32 under turbo,
// so the top-k selected rows are read straight from device memory and
// staged identically to the fp32 variant above.
kernel void kernel_dsv4_indexed_mixed_attention_heads8_turbo(
        constant ds4_metal_args_dsv4_indexed_attention_turbo & args,
        device const char *q,
        device const char *raw_kv,
        device const char *comp_kv,
        device const char *topk,
        ...
        device       char *dst,
        threadgroup float4 *kv_shared [[threadgroup(0)]],
        threadgroup float  *norms_tg  [[threadgroup(1)]],
        uint2  tgpig [[threadgroup_position_in_grid]],
        ushort tid   [[thread_index_in_threadgroup]],
        ushort lane  [[thread_index_in_simdgroup]],
        ushort sg    [[simdgroup_index_in_threadgroup]]) {
    ...
    threadgroup float *row_flat = (threadgroup float *)kv_shared;

    if (first <= last) {
        for (uint pos = first; pos <= last; pos++) {
            const uint logical = pos - first_raw_pos;
            const uint row = (args.raw_start + logical) % args.raw_cap;
            device const uchar *row_in = (device const uchar *)(raw_kv +
                (uint64_t)row * args.raw_row_stride);

            // Inline-decode this row into kv_shared. All 256 threads
            // participate in the helper to keep its threadgroup
            // barriers convergent.
            turbo_decode_row_into_shared_512(row_in,
                                             row_flat,
                                             norms_tg,
                                             args.bits,
                                             args.n_rot,
