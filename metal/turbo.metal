...
// 256-thread variant of turbo_dequant_group used by attention kernels
// that share a 32x8 threadgroup. Only the first 128 lanes participate in
// the math; the upper 128 lanes still execute every barrier so the call
// is convergent for the whole threadgroup. The decoded floats land in
// rot[0..127] (caller's threadgroup buffer slice for this rotation group).
static void turbo_dequant_group_256(
        device const uchar      * row_in,
        threadgroup float       * rot,        // 128-element slice for this group
        threadgroup float       * norms_tg,   // 4 floats, scratch
        int g,
        int bits,
        ushort tid) {
    const int blocks_pg = TURBO_ROT_D / TURBO_BLOCK;
    ...
    const ushort active = (tid < (ushort)TURBO_ROT_D) ? (ushort)1 : (ushort)0;
    const ushort b    = tid >> 5;        // 0..3 (only meaningful for tid<128)
    const ushort lane = tid & 31;

    const int block_idx = g * blocks_pg + b;
    device const uchar * blk = row_in + (uint)block_idx * blk_bytes;

    // Block norm: one writer per block staging into norms_tg[b].
    if (active && lane == 0) {
        const ushort n16 = (ushort)blk[0] | ((ushort)blk[1] << 8);
        norms_tg[b] = turbo_f16_to_f32_bits(n16);
    }
    ...
    // Codebook lookup and write into rot.
    if (active) {
        const float inv_scale = norms_tg[b] * (1.0f / sqrt((float)TURBO_BLOCK));
        const uchar qs_byte = blk[2 + (lane >> 2)];
        const uchar sg_byte = blk[10 + (lane >> 3)];
    ...
    // Inverse rotation: signs2 -> FWHT-128 -> signs1, with a final
    // 1/sqrt(128) folded into the FWHT helper. All 256 threads execute
    // the barriers; only tid<128 touches rot.
    if (active) {
        rot[tid] *= turbo_wht_signs2[tid];
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    // Inline FWHT-128 with explicit gating so the upper 128 lanes still
    // reach every barrier. Mirrors turbo_fwht_128_tg() exactly.
    for (ushort h = 1; h < TURBO_ROT_D; h <<= 1) {
        float a = 0.0f;
        float b_v = 0.0f;
        ushort pair = 0;
        ...
        threadgroup_barrier(mem_flags::mem_threadgroup);
        if (active) {
            rot[tid] = (tid < pair) ? (a + b_v) : (b_v - a);
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
    if (active) {
        rot[tid] *= 0.08838834764831845f; // 1 / sqrt(128)
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
...
// Decode one turbo-encoded row directly into a 512-float threadgroup
// buffer in the layout the attention kernel expects: 4 contiguous
// 128-float groups, then a fp16 RoPE tail overlay on the last n_rot
// dims. Used by kernel_dsv4_indexed_mixed_attention_heads8_turbo.
//
// Layout invariants:
//   row_in            : pointer to start of this row's turbo bytes
//                       (turbo prefix = blocks_per_row * blk_bytes,
//                        then n_rot * 2 bytes of fp16 RoPE tail).
//   row_flat[0..511]  : output, float per dim (caller's kv_shared aliased).
//   norms_tg[0..3]    : scratch for per-block norm broadcast.
//   bits              : 3 or 4.
//   n_rot             : RoPE tail dims (e.g. 64).
//   tid               : 0..255.
static void turbo_decode_row_into_shared_512(
        device const uchar      * row_in,
        threadgroup float       * row_flat,
        threadgroup float       * norms_tg,
        int bits,
        int n_rot,
        ushort tid) {
    const int n_groups = 4; // head_dim 512 / TURBO_ROT_D 128
    for (int g = 0; g < n_groups; g++) {
        turbo_dequant_group_256(row_in,
                                row_flat + (uint)g * (uint)TURBO_ROT_D,
                                norms_tg,
                                g,
                                bits,
                                tid);
    ...
    // Apply the RoPE tail: overwrite the last n_rot dims with the fp16
    // verbatim copy stored after the turbo prefix.
    if (n_rot > 0 && (int)tid < n_rot) {
        const int blocks_pg = TURBO_ROT_D / TURBO_BLOCK;
        const uint blk_bytes = (bits == 3) ? 14u : 18u;
        const uint turbo_bytes = (uint)blk_bytes * (uint)(n_groups * blocks_pg);
        device const uchar * tail = row_in + turbo_bytes;
        const ushort lo = (ushort)tail[(uint)tid * 2 + 0];
        const ushort hi = (ushort)tail[(uint)tid * 2 + 1];
        const ushort h  = lo | (hi << 8);
        const uint nope = (uint)(512 - n_rot);
        row_flat[nope + (uint)tid] = (float)as_type<half>(h);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
}
